...
%		TRAINDistribution: training labels
%
%   Outputs,
...
size(trainDistribution)
size(log(modProb))
%A=(log(modProb))';
%size(A)
%log(modProb)=(log(modProb))';
avgavglogliklihood = -sum(sum(trainDistribution.*log(modProb)));
%avgavglogliklihood = -sum(sum(trainDistribution.*A));


function bd = betader(delta, modProb, MU, trainDistribution, xSum)
% The equation of delta.
item1 = trainDistribution'*MU;
item2 = sum(modProb.*MU.*exp(delta.*sign(MU).*xSum));
bd = item1-item2;
