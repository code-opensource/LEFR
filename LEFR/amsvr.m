...
%   from the original algorithm msvr writed by Fernando P閞ez Cruz. We modify it 
...
while(hacer)
    ...
    M11=[para.C1*H(i1,i1)+diag(1./a(i1))]; %C1*H+I
    M12 = para.C1*ones(size(M11,1),1); %C1*H^T*a
    M21 = para.C1*a(i1)'*H(i1,i1);  %C1*a^T*H,H = kernelmatrix(para.ker, para.par, X, X);
    M22 = para.C1*sum(a(i1)); %C1*1^T*a
    M = [M11 M12; M21 M22];
    M=M+1e-11*eye(size(M,1));

    %compute betas
	YS_a(i2) = 0;
    M1 = para.C1*Y(i1,:) + para.C2*YS_a(i1,:)./repmat(a(i1), 1, L); %size(Y(i1,:))=320,19（N*L）
    %i1 = find(u>=para.epsi);预测误差大于epsilon的点，即找到其损失函数！= 0的SV
    %Y:数字标签矩阵 mu（N×L）,Y(i1,:)=mu^j
    M2 = para.C1*(a(i1)'*Y(i1,:))+para.C2*sum(YS_a(i1,:)); %size(a(i1)')=1,320 size(Y(i1,:))=320,L   
    %size((a(i1)'*Y(i1,:)))=1,L
    sal1=inv(M)*[M1; M2];
    b_sal1=sal1(end,:);
    sal1=sal1(1:end-1,:);
